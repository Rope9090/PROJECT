...
# Secondary database connection for cooldown times
cooldown_conn = sqlite3.connect("cooldown.db")
...
"""
cooldown_cursor.execute(create_cooldown_table_sql)
cooldown_conn.commit()
cooldown_conn.close()
...
class Face_Recognizer:
    ...
    def attendance(self, name):
        ...
        # Check cooldown
        cooldown_cursor.execute("SELECT next_check_out FROM cooldowns WHERE name=?", (name,))
        result = cooldown_cursor.fetchone()

        if result:
            next_check_out = result[0]
            if timestamp < next_check_out:
                print(f"Cooldown period not over for {name}. Skipping attendance.")
                cooldown_conn.close()
                return

        # Record attendance (in primary database)
        conn = sqlite3.connect("attendance1.db")
        cursor = conn.cursor()

        cursor.execute("SELECT * FROM attendance1 WHERE name=? AND date=?", (name, current_date))
        existing_entry = cursor.fetchone()

        if existing_entry:
            # Update check-out time
            cursor.execute("UPDATE attendance1 SET check_out=? WHERE name=? AND date=?",
                           (current_time, name, current_date))
            print(f"{name} checked out at {current_time}")
        else:
            # Insert new check-in record
            cursor.execute("INSERT INTO attendance1 (name, check_in, check_out, date) VALUES (?, ?, ?, ?)",
                           (name, current_time, None, current_date))
            print(f"{name} checked in at {current_time}")

        conn.commit()
        conn.close()

        # Update cooldown (in secondary database)
        next_check_out = timestamp + self.cooldown_time
        cooldown_cursor.execute("INSERT OR REPLACE INTO cooldowns (name, next_check_out) VALUES (?, ?)",
                                (name, next_check_out))
        cooldown_conn.commit()
        cooldown_conn.close()
